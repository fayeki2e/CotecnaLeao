...
*/
    function renderParsedFormat(parsedFormat, date1, date2, separator, isRTL) {
      ...
      var unzonedDate1 = date1.clone()
        .stripZone() // for same-unit comparisons
      var unzonedDate2 = date2.clone()
        .stripZone() // "
...
*/
    function renderFakeFormatString(fakeFormatString, date) {
      return processMaybeMarkers(
        renderFakeFormatStringParts(fakeFormatString, date)
          .join('')
      )
    }
...
// Can be called with another class, or a plain object hash containing new members.
  ...
  var EmitterMixin = FC.EmitterMixin = {
    ...
    on: function (types, handler) {
      $(this)
        .on(types, this._prepareIntercept(handler))
      return this // for chaining
    },
    ...
    one: function (types, handler) {
      $(this)
        .one(types, this._prepareIntercept(handler))
      return this // for chaining
    },
    ...
    trigger: function (types) {
      ...
      $(this)
        .triggerHandler(types, { args: args })
...
*/
  ...
  var Popover = Class.extend(ListenerMixin, {
    ...
    position: function () {
      var options = this.options
      var origin = this.el.offsetParent()
        .offset()
  ...
  Grid.mixin({
    ...
    bindSegHandlerToEl: function (el, name, handler) {
      ...
      el.on(name, this.segSelector, function (ev) {
        var seg = $(this)
          .data('fc-seg') // grab segment data. put there by View::renderEvents
    ...
    startSegResize: function (seg, ev, dragOptions) {
      if ($(ev.target)
        .is('.fc-resizer')) {
        this.buildSegResizeListener(seg, $(ev.target)
          .is('.fc-start-resizer'))
    ...
    computeExternalDrop: function (span, meta) {
      ...
      if (meta.duration) {
        dropLocation.end = dropLocation.start.clone()
          .add(meta.duration)
      }
      ...
      var dragListener = this.segResizeListener = new HitDragListener(this, {
        ...
        hitOver: function (hit, isOrig, origHit) {
          ...
          if (!isAllowed) {
            ...
            if (
              resizeLocation.start.isSame(event.start.clone()
                .stripZone()) &&
              resizeLocation.end.isSame(eventEnd.clone()
                .stripZone())
            ) {
    ...
    computeEventResize: function (type, startSpan, endSpan, event) {
      ...
      if (!resizeLocation.start.isBefore(resizeLocation.end)) {
        ...
        if (type == 'start') { // resizing the start?
          resizeLocation.start = resizeLocation.end.clone()
            .subtract(defaultDuration)
        }
        else { // resizing the end?
          resizeLocation.end = resizeLocation.start.clone()
            .add(defaultDuration)
        }
      }
...
*/
  var DayTableMixin = FC.DayTableMixin = {
    ...
    sliceRangeByRow: function (range) {
      ...
      var rangeLast = this.getDateDayIndex(normalRange.end.clone()
        .subtract(1, 'days')) // inclusive last index
      var segs = []
      var row
      var rowFirst,
        rowLast // inclusive day-index range for current row
      var segFirst,
        segLast // inclusive day-index range for segment
    ...
    renderNumberCellHtml: function (date) {
      ...
      if (view.cellWeekNumbersVisible) {
        ...
        if (date._locale._fullCalendar_weekCalc === 'ISO') {
          weekCalcFirstDoW = 0  // Monday by ISO 8601 definition
        }
        else {
          weekCalcFirstDoW = 0//date._locale.firstDayOfWeek()
        }
      }
    ...
    renderHelper: function (event, sourceSeg) {
      ...
      this.rowEls.each(function (row, rowNode) {
        ...
        if (sourceSeg && sourceSeg.row === row) {
          ...
          skeletonTop = rowEl.find('.fc-content-skeleton tbody')
            .position().top
        }
  ...
  DayGrid.mixin({
    ...
    computeRowLevelLimit: function (row) {
      ...
      function iterInnerHeights(i, childNode) {
        trHeight = Math.max(trHeight, $(childNode)
          .outerHeight())
      }
      ...
      for (i = 0; i < trEls.length; i++) {
        ...
        trHeight = 0
        trEl.find('> td > :first-child')
          .each(iterInnerHeights)
    ...
    renderSegPopoverContent: function (row, col, segs) {
      ...
      var title = this.getCellDate(row, col)
        .format(view.opt('dayPopoverFormat'))
      var content = $(
    ...
    sliceRangeByTimes: function (range) {
      ...
      for (dayIndex = 0; dayIndex < this.daysPerRow; dayIndex++) {
        dayDate = this.dayDates[dayIndex].clone()
          .time(0) // TODO: better API for this?
        dayRange = {
          start: dayDate.clone()
            .add(this.view.minTime), // don't use .time() because it sux with negatives
          end: dayDate.clone()
            .add(this.view.maxTime)
        }
  ...
  var View = FC.View = Model.extend({
    ...
    formatRange: function (range, formatStr, separator) {
      ...
      if (!end.hasTime()) { // all-day?
          ...
          .subtract(1) // convert to inclusive. last ms of previous day
      }
    ...
    startNowIndicator: function () {
      ...
      if (this.opt('nowIndicator')) {
        ...
        if (unit) {
          ...
          delay = this.initialNowDate.clone()
            .startOf(unit)
    ...
    computeDayRange: function (range) {
      var startDay = range.start.clone()
        .stripTime() // the beginning of the day the range starts
      var end = range.end
      var endDay = null
      var endTimeMS
      ...
      if (end) {
        endDay = end.clone()
          .stripTime() // the beginning of the day the range exclusively ends
  ...
  var Calendar = FC.Calendar = Class.extend(EmitterMixin, {
    ...
    changeView: function (viewName, dateOrRange) {
      ...
      if (dateOrRange) {
        ...
        if (dateOrRange.start && dateOrRange.end) { // a range
          ...
          this.currentDate = this.moment(dateOrRange)
    ...
    initCurrentDate: function () {
      ...
      if (defaultDateInput != null) {
        this.currentDate = this.moment(defaultDateInput)
          .stripZone()
      }
      else {
    ...
    gotoDate: function (zonedDateInput) {
      this.currentDate = this.moment(zonedDateInput)
        .stripZone()
      this.renderView()
    },
    ...
    buildSelectSpan: function (zonedStartInput, zonedEndInput) {
      var start = this.moment(zonedStartInput)
        .stripZone()
      var end

      if (zonedEndInput) {
        end = this.moment(zonedEndInput)
          .stripZone()
      }
...
*/
  ...
  Calendar.mixin({
    ...
    applyTimezone: function (date) {
      ...
      if (timeAdjust) { // is the time result different than expected?
        adjustedZonedDate = zonedDate.clone()
          .add(timeAdjust) // add milliseconds
    ...
    humanizeDuration: function (duration) {
      return duration.locale(this.opt('locale'))
        .humanize()
    },
  ...
  Calendar.mixin({
    ...
    initialRender: function () {
      ...
      this.contentEl = $('<div class=\'fc-view-container\'/>')
        .prependTo(el)
    ...
    _calcSize: function () { // assumes elementVisible
      ...
      if (typeof contentHeightInput === 'number') { // exists and not 'auto'
        ...
        this.suggestedViewHeight = this.el.parent()
          .height() - this.queryToolbarsHeight()
      }
      else {
    ...
    function expandEvent(abstractEvent, _rangeStart, _rangeEnd) {
      ...
      if (abstractEvent) {
        if (abstractEvent._recurring) {
          ...
          date = _rangeStart.clone()
            .stripTime() // holds the date of the current day
...
// fully contain the other?
  Calendar.prototype.spanContainsSpan = function (outerSpan, innerSpan) {
    var eventStart = outerSpan.start.clone()
      .stripZone()
    var eventEnd = this.getEventEnd(outerSpan)
      .stripZone()
...
// start/end already assumed to have stripped zones :(
  Calendar.prototype.eventIntersectsRange = function (event, range) {
    var eventStart = event.start.clone()
      .stripZone()
    var eventEnd = this.getEventEnd(event)
      .stripZone()
...
// It is responsible for managing width/height.
  ...
  var BasicView = FC.BasicView = View.extend({
    ...
    buildRenderRange: function (currentRange, currentRangeUnit) {
      ...
      if (/^(year|month)$/.test(currentRangeUnit)) {
        ...
        if (renderRange.end.weekday()) {
          renderRange.end.add(1, 'week')
            .startOf('week') // exclusively move backwards
        }
      }
    ...
    renderDates: function () {
      ...
      this.el.addClass('fc-basic-view')
        .html(this.renderSkeletonHtml())
      this.renderHead()

      this.scroller.render()
      var dayGridContainerEl = this.scroller.el.addClass('fc-day-grid-container')
      var dayGridEl = $('<div class="fc-day-grid" />')
        .appendTo(dayGridContainerEl)
      this.el.find('.fc-body > tr > td')
        .append(dayGridContainerEl)
...
----------------------------------------------------------------------------------------------------------------------*/
  ...
  var YearView = FC.YearView = View.extend({
    ...
    updateCells: function () {
      ...
      this.subTables.find('.fc-week:first')
        .addClass('fc-first')
      this.subTables.find('.fc-week:last')
        .addClass('fc-last')
      this.subTables.find('.fc-bg')
        .find('td .fc-day:last')
        .addClass('fc-last')
    ...
    buildSkeleton: function (monthsPerRow, showNumbers) {
      ...
      for (n = 0; n < 7; n++) {
        weekNames[n] = FC.moment([2013, 11, 1 + n])
          .format('ddd')
      }
      ...
      this.head.find('tr.fc-year-week-days th.fc-year-weekly-head:first')
        .addClass('fc-first')
      this.head.find('tr.fc-year-week-days th.fc-year-weekly-head:last')
        .addClass('fc-last')

      this.table.find('.fc-year-monthly-name a')
        .click(this.calendar, function (ev) {
          ev.data.changeView('month')
          ev.data.gotoDate([$(this)
            .attr('data-year'), $(this)
            .attr('data-month'), 1])
        })
    ...
    buildDayGrids: function () {
      ...
      var baseDate = view.intervalStart.clone()
        .add(7, 'days') // to be sure we are in month
      view.dayGrids = []
...
// Responsible for managing width/height.
  ...
  var AgendaView = FC.AgendaView = View.extend({
    ...
    renderDates: function () {
      ...
      this.el.addClass('fc-agenda-view')
        .html(this.renderSkeletonHtml())
      this.renderHead()

      this.scroller.render()
      var timeGridWrapEl = this.scroller.el.addClass('fc-time-grid-container')
      var timeGridEl = $('<div class="fc-time-grid" />')
        .appendTo(timeGridWrapEl)
      this.el.find('.fc-body > tr > td')
        .append(timeGridWrapEl)
...
*/
  var ListViewGrid = Grid.extend({
    ...
    spanToSegs: function (span) {
      var view = this.view
      var dayStart = view.renderRange.start.clone()
        .time(0) // timed, so segs get times!
      var dayIndex = 0
      var seg
      var segs = []
      ...
      while (dayStart < view.renderRange.end) {
        ...
        if (
          ...
          span.end < dayStart.clone()
            .add(this.view.nextDayThreshold)
        ) {
      ...
      for (dayIndex = 0; dayIndex < segsByDay.length; dayIndex++) {
        ...
        if (daySegs) { // sparse array, so might be undefined
          ...
          tbodyEl.append(this.dayHeaderHtml(
            this.view.renderRange.start.clone()
              .add(dayIndex, 'days')
          ))
